angular.module('myApp.dayFoodCurve', 
		['ionic','ngCordova','ngTouch',
		 'ui.grid','ui.grid.pinning','ui.grid.edit','ui.grid.cellNav', 'ui.grid.validate', 'ui.grid.grouping', 'ui.grid.selection','ui.grid.resizeColumns','ui.grid.autoResize'
		 ])

.controller('dayFoodCurveCtrl', function($scope, $state) {
			...
			Sparraw.ajaxPost('/rep/DailyFeed/DFRRep.action', params, function(data){
				console.log("批次号对比");
						if (data.ResponseDetail.Result == "Success" && data.ResponseDetail.DCDatas.length != 0){
							 ...
							 var ageLeng =[];
							 //判断是否有标准信息
							if (data.ResponseDetail.DCDatas[0].HouseDatas.length == 0) {
								data.ResponseDetail.DCDatas.shift();
							}
