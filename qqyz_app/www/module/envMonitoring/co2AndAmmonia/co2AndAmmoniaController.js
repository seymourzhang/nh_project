 angular.module('myApp.co2AndAmmonia', 
        ['ionic','ngCordova','ngTouch',
         'ui.grid','ui.grid.pinning','ui.grid.edit','ui.grid.cellNav', 'ui.grid.validate', 'ui.grid.grouping', 'ui.grid.selection','ui.grid.resizeColumns','ui.grid.autoResize'
         ])
 // 二氧化碳与氨气
.controller("co2AndAmmoniaCtrl",function($scope, $state,$ionicLoading, $http, $ionicPopup, $ionicScrollDelegate, $stateParams, $ionicScrollDelegate, AppData){
	...
	$scope.getChart = function(FarmBreedId,HouseId,DataType,ReqFlag,DataRange){
		...
		Sparraw.ajaxPost('tempMobile/CO2_NH4_CurveReq', params, function(data){
	...
	$scope.setChartParams = function(data){
		...
		for (var i = 0; i < data.ResponseDetail.TempDatas.length; i++) {
			var intoyType      = "" ;
			var intoyAxisIndex = "" ;
			if (data.ResponseDetail.TempDatas[i].TempAreaName == "氨气") {
				intoyType      = "line" ;
				intoyAxisIndex = 1      ;
			}else{
				intoyType      = "line" ;
