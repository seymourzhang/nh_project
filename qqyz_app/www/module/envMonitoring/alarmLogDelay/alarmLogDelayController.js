...
.controller("alarmLogDelayCtrl",function($scope, $state, $http,  crisisServiceFactory,  $ionicPopup, $stateParams, $ionicActionSheet, AppData) {
	...
	$scope.alarmQuery = function(){
		...
		Sparraw.ajaxPost('alarmMobile/alarmDealQuery', params, function(data){
			if (data.ResponseDetail.Result == "Success") {
        		...
        		$scope.delNullData($scope.logDelayData.alarmMessage);
	...
	$scope.delNullData = function(serverGetData){
		var screeningNullDataArry = [];
		for (var i = 0; i < serverGetData.length; i++) {
			if (serverGetData[i].CurAlarmData.length != 0) {
				screeningNullDataArry.push(serverGetData[i]);
			}
		}
		$scope.logDelayData.alarmMessage = screeningNullDataArry;
	}
