...
.controller('warnStatisticsCtrl', function($scope, $state) {
	...
	$scope.getAlarmStatic = function(selectedHouse){
		...
		var params = {};
		params.HouseId = selectedHouse;
		params.FarmId = $scope.sparraw_user_temp.farminfo.id;
		if (selectedHouse == 0) {
			params.BreedBatchNo = $scope.sparraw_user_temp.houseinfos[0].BreedBatchNo;
		}else{
			for (var i = 0; i < $scope.sparraw_user_temp.houseinfos.length; i++) {
				if ($scope.sparraw_user_temp.houseinfos[i].id == selectedHouse) {
					params.BreedBatchId = $scope.sparraw_user_temp.houseinfos[i].BreedBatchId;
					params.BreedBatchNo = $scope.sparraw_user_temp.houseinfos[i].BreedBatchNo;
				}
			}
		}
		

		Sparraw.ajaxPost('alarmMobile/alarmStatic.action', params, function(data){
