...
.controller("landingPageCtrl",function($scope, $state, $http, AppData, $stateParams, $ionicModal) {
    ...
    $scope.chooseServer = function(){
    	...
    	if(CONFIG_AppMode == 'Production'){
    		API_Host = CONFIG_Production_API_Host ;
    	}else if(CONFIG_AppMode == 'Development'){
	    	API_Host = CONFIG_Development_API_Host ;
	    }else if(CONFIG_AppMode == 'Local'){
	    	API_Host = CONFIG_Local_API_Host ;
	    }

	    if(CONFIG_LogInMode == 'Public'){
	    	var params = {
				"userCode": $scope.landing.userCode,
			};
		 	
		 	Sparraw.ajaxPost('ws/getUserInfoForMobile/', params, function(data){
			    if (data.ResponseDetail.Result == 'Success') {
			    	console.log('账户所属服务地址：' + data.ResponseDetail.ServerHost);
			    	API_Host = data.ResponseDetail.ServerHost;
			    	persistentData.ServerName = data.ResponseDetail.ServerName;
			    	$scope.accountLogin();
			   	}else{
			   		API_Host = null;
			   		Sparraw.myNotice("系统中暂无此用户。");
			   	};
			});
	    }else if(CONFIG_LogInMode == 'Private'){
	    	$scope.accountLogin();
	    }else{
	    	app_alert('CONFIG_LogInMode 参数配置错误 ');
	    }
    }
    ...
    $scope.clickQueryDetail = function(farmId){
	    ...
	    Sparraw.ajaxPost('loginMobile/queryDetail', params, function(data){
	    	if (data.ResponseDetail.Result == 'Success') {
	    		if (!data.ResponseDetail.FarmInfo) {return app_alert("请先添加农场信息");}
				...
				$scope.sparraw_user_temp = JSON.parse(JSON.stringify(sparraw_user));//将获取到的最新数据赋值
				try{
					var jpushTags = [];
					// 如果用户设置了栋舍信息
