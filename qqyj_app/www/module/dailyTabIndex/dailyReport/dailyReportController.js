...
.controller("dailyReportCtrl",function($scope, $state,$ionicLoading, $http, $ionicPopup, $ionicScrollDelegate, $stateParams, $ionicScrollDelegate, AppData){
    ...
    $scope.inquire = function() {
    	...
    	for (var i = 0; i < $scope.sparraw_user_temp.houseinfos.length; i++) {//获取批次号以及栋舍状态
			if ($scope.sparraw_user_temp.houseinfos[i].id == $scope.dailyReportData.HouseId) {
				$scope.dailyReportData.HouseBreedId = $scope.sparraw_user_temp.houseinfos[i].BreedBatchId;
				$scope.dailyReportData.BreedBatchStatus = $scope.sparraw_user_temp.houseinfos[i].BreedBatchStatus;
			}
		}
		if ($scope.dailyReportData.selectDayAge == "" || !$scope.dailyReportData.selectDayAge) {
			$scope.dailyReportData.selectDayAge = (new Date()).Format("yyyy-MM-dd");
		}
			...
			//接受数据
	   		if (data.ResponseDetail.Result == "Success") {
	   			...
	   			$scope.dailyReportData.inputData = {
					...
					"layer_qual_amount"     :  data.ResponseDetail.DataInfo.layer_qual_amount      ,  //合格蛋数量
					"layer_qual_weight"     :  data.ResponseDetail.DataInfo.layer_qual_weight         //合格蛋均重
	   			};
	...
	$scope.judgeHouse = function(){
		...
		if ($scope.sparraw_user_temp.farminfo.farm_type_id == "2") {
	...
	$scope.save = function(){
    	...
    	$scope.dailyReportData.inputData.death_num_male        = Common_NulltoZero($scope.dailyReportData.inputData.death_num_male       );
    	$scope.dailyReportData.inputData.death_num_female      = Common_NulltoZero($scope.dailyReportData.inputData.death_num_female     );
    	$scope.dailyReportData.inputData.culling_num_male      = Common_NulltoZero($scope.dailyReportData.inputData.culling_num_male     );
    	$scope.dailyReportData.inputData.culling_num_female    = Common_NulltoZero($scope.dailyReportData.inputData.culling_num_female   );
    	$scope.dailyReportData.inputData.body_weight_male      = Common_NulltoZero($scope.dailyReportData.inputData.body_weight_male     );
    	$scope.dailyReportData.inputData.body_weight_female    = Common_NulltoZero($scope.dailyReportData.inputData.body_weight_female   );
    	$scope.dailyReportData.inputData.gender_error_male     = Common_NulltoZero($scope.dailyReportData.inputData.gender_error_male    );
    	$scope.dailyReportData.inputData.gender_error_female   = Common_NulltoZero($scope.dailyReportData.inputData.gender_error_female  );
    	$scope.dailyReportData.inputData.feed_code_female      = Common_NulltoZero($scope.dailyReportData.inputData.feed_code_female     );
    	$scope.dailyReportData.inputData.feed_weight_female    = Common_NulltoZero($scope.dailyReportData.inputData.feed_weight_female   );
    	$scope.dailyReportData.inputData.water_capacity_female = Common_NulltoZero($scope.dailyReportData.inputData.water_capacity_female);
    	$scope.dailyReportData.inputData.layer_amount          = Common_NulltoZero($scope.dailyReportData.inputData.layer_amount         );
    	$scope.dailyReportData.inputData.layer_qual_amount     = Common_NulltoZero($scope.dailyReportData.inputData.layer_qual_amount    );
    	$scope.dailyReportData.inputData.layer_qual_weight     = Common_NulltoZero($scope.dailyReportData.inputData.layer_qual_weight    );
    	$scope.dailyReportData.inputData.uniformity            = Common_NulltoZero($scope.dailyReportData.inputData.uniformity           );

		var params = {
			"FarmId"                 :  $scope.dailyReportData.FarmId                           ,
			...
			"layer_qual_amount"      :  $scope.dailyReportData.inputData.layer_qual_amount      ,   //合格蛋数量
			"layer_qual_weight"      :  $scope.dailyReportData.inputData.layer_qual_weight      ,   //合格蛋均重
	...
	$scope.getHouseListFun = function(){
		console.log($scope.sparraw_user_temp);
		console.log($scope.sparraw_user_temp.userinfos);
		for (var i = 0; i < $scope.sparraw_user_temp.userinfos.length; i++) {//获取该用户下的所有栋舍id
			if ($scope.sparraw_user_temp.userinfo.userId == $scope.sparraw_user_temp.userinfos[i].userId) {
				for (var j = 0; j < $scope.sparraw_user_temp.userinfos[i].UserHouse.length; j++) {
					$scope.dailyReportData.HouseList = $scope.sparraw_user_temp.userinfos[i].UserHouse;
				}
			}
		}
		$scope.dailyReportData.selectHouse = $scope.dailyReportData.HouseList[0];
		$scope.judgeHouse();
	}
