...
.controller("dailyCtrl",function($scope, $state, $http, $stateParams, $ionicPopup, AppData) {
    ...
    $scope.GetTable = function(){
        ...
        var index11MaxLength = [];
        var index12MaxLength = [];
        ...
        for (var i = 0; i < $scope.prodDayData.weekData.length; i++) {
          ...
          index11MaxLength.push(String($scope.prodDayData.weekData[i].index11).length);
          index12MaxLength.push(String($scope.prodDayData.weekData[i].index12).length);
        }
        ...
        if (headName[10].length < index11MaxLength.max()) {
          allDataLength.push(index11MaxLength.max());
        }else{
          allDataLength.push(headName[10].length);
        }

        if (headName[11].length < index12MaxLength.max()) {
          allDataLength.push(index12MaxLength.max());
        }else{
          allDataLength.push(headName[11].length);
        }
          ...
          if (data.ResponseDetail.Result == "Success") {
            ...
            for (var i = 0; i < data.ResponseDetail.dataInfo.length; i++) {
              $scope.prodDayData.weekData.push({
                  ...
                  "index2":data.ResponseDetail.dataInfo[i].female_cur_amount   ,
                  "index3":data.ResponseDetail.dataInfo[i].male_cur_amount     ,
                  "index4":data.ResponseDetail.dataInfo[i].male_cd_num         ,
                  "index5":data.ResponseDetail.dataInfo[i].female_cd_num       ,
                  "index6":data.ResponseDetail.dataInfo[i].intake_sig          ,
                  "index7":data.ResponseDetail.dataInfo[i].water_sig           ,
                  "index8":data.ResponseDetail.dataInfo[i].male_body_weight    ,
                  "index9":data.ResponseDetail.dataInfo[i].female_body_weight  ,
                  "index10":data.ResponseDetail.dataInfo[i].layer_num          ,
                  "index11":data.ResponseDetail.dataInfo[i].layer_rate         ,
                  "index12":data.ResponseDetail.dataInfo[i].uniformity
              })
            }
            $scope.GetTable();
