 angular.module('myApp.updateDevice', 
        ['ionic','ngCordova','ngTouch',
         'ui.grid','ui.grid.pinning','ui.grid.edit','ui.grid.cellNav', 'ui.grid.validate', 'ui.grid.grouping', 'ui.grid.selection','ui.grid.resizeColumns','ui.grid.autoResize'
         ])
 // 栋舍修改
.controller("updateDeviceCtrl",function($scope, $state, $http, $stateParams, AppData,$ionicPopup) {
	...
	$scope.startEdit = function(){
  	...
  	$scope.dealBarCode = function(){
		Common_barScan(function(result){
			$scope.deviceData.device_code = result.text;
	...
	$scope.saveUpdate = function(){
		...
		if ($scope.deviceData.device_code == "") {
			app_confirm('请确认，是否解绑设备。','提示',null,function(buttonIndex){
			   	if(buttonIndex == 2){
