...
.controller("updateTempCtrl",function($scope, $state, $http, $stateParams, AppData) {
    ...
    $scope.getHighCautionTemp = function(highValue){
        if (!isNaN(highValue) || highValue != 0) {
            $scope.updateTempData.tableData.high_caution_temp = (highValue * 0.90).toFixed(1);
        }else{
    ...
    $scope.getlowCautionTemp = function(lowValue){
        if (!isNaN(lowValue) || lowValue != 0) {
            $scope.updateTempData.tableData.low_caution_temp = (lowValue * 1.1).toFixed(1);
        }else{
    ...
    $scope.inspectionParams = function(params){
        ...
        if (isNaN(params.low_caution_temp)) {
            app_alert("请输入正确的低报警示");
            return false;
        }

        if (isNaN(params.high_caution_temp)) {
            app_alert("请输入正确的高报警示");
            return false;
        }
