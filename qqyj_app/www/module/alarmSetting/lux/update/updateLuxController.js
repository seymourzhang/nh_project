...
.controller("updateLuxCtrl",function($scope, $state, $http, $stateParams, AppData) {
    ...
    $scope.getHighCautionLux = function(highLuxValue){
        if (!isNaN(highLuxValue) || highLuxValue != 0) {
            $scope.updateLuxData.tableData.high_caution_lux = (highLuxValue * 0.90).toFixed(1);
        }else{
    ...
    $scope.getLowCautionLux = function(lowLuxValue){
        if (!isNaN(lowLuxValue) || lowLuxValue != 0) {
            $scope.updateLuxData.tableData.low_caution_lux = (lowLuxValue * 1.1).toFixed(1);
        }else{
    ...
    $scope.inspectionParams = function(params){
        ...
        if (isNaN(params.high_caution_lux)) {
            app_alert("请输入正确的高报警示值。");
            return false;
        }
        if (isNaN(params.low_caution_lux)) {
            app_alert("请输入正确的低报警示值。");
            return false;
        }
