...
.controller("dailyDayCtrl",function($scope, $state, $http, $ionicModal,$stateParams, $ionicPopup, AppData) {
	...
	if (DeviInfo.ScreenHeight > DeviInfo.ScreenWidth) {
		document.getElementById('dailyDay_DIV').style.height = (DeviInfo.ScreenHeight - 100) + 'px';
	}else{
		document.getElementById('dailyDay_DIV').style.height = (DeviInfo.ScreenWidth - 100) + 'px';
	}
	...
	$scope.GetDayTable = function(){
		...
		var toDayTitleName = ["日龄","死淘","死淘率","标准","采食","只日耗","标准","饮水","水料比"];
		toDayTableKey = ["day_age",
							"dc_num",
							"dc_rate_actual",
							"dc_rate_standard",
		...
		//表主体部分
		  for (var i = 0; i < showTableData.header.length; i++) {
		  	if (i == 0 || i == 4 ||i == 3 || i == 6 || i == 8 ||i == 9 ||i == 11) {
	...
	$scope.chooseHouse = function(item){
		for (var i = 0; i < $scope.sparraw_user_temp.houseinfos.length; i++) {
			document.getElementById("IDis"+$scope.sparraw_user_temp.houseinfos[i].id).style.background = "#C6A87B";
		}
		document.getElementById("IDis"+item).style.background = "#E3E3E3";
		$scope.dailyDayData.selectHouse.HouseId = item;
		$scope.inquire();
	}
