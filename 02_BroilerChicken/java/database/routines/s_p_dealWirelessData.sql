  ...
  label_at_start:BEGIN
    ...
    */
        ...
        SELECT
          ...
          null as avg_temp,
          iFNULL(b.set_temp, (SELECT z.set_temp
                              FROM s_b_dayage_temp_sub z
                              WHERE a.id = z.uid_num
                              ORDER BY z.record_datetime DESC
                              LIMIT 1))                 AS set_temp,
          ifnull(b.high_alarm_temp, (SELECT z.high_alarm_temp
                                     FROM s_b_dayage_temp_sub z
                                     WHERE a.id = z.uid_num
                                     ORDER BY z.record_datetime DESC
                                     LIMIT 1))          AS high_alarm_temp,
          ifnull(b.low_alarm_temp, (SELECT z.low_alarm_temp
                                    FROM s_b_dayage_temp_sub z
                                    WHERE a.id = z.uid_num
                                    ORDER BY z.record_datetime DESC
                                    LIMIT 1))           AS low_alarm_temp,
          ...
          ifnull(b.set_humidity, (SELECT z.set_humidity
                                  FROM s_b_dayage_temp_sub z
                                  WHERE a.id = z.uid_num
                                  ORDER BY z.record_datetime DESC
                                  LIMIT 1))             AS set_humidity,
        ...
        from s_b_monitor_curr_wireless01 sbw
          ...
          LEFT JOIN s_b_house_alarm sba on sba.house_id = sbw.houseid
        ) as a;
