        ...
        try {
            ...
            if (HouseId != 0) {
                SQL = "SELECT " +
                        ...
                        "    THEN 1 " +
                        "      ELSE 0 END)               AS co2 " +
                        "FROM s_b_breed_detail bd LEFT JOIN s_b_house_breed hb ON hb.id = bd.house_breed_id " +
                        ...
                        "    ON ad.house_id = hb.house_id AND datediff(ad.alarm_time, bd.growth_date) = 0 " +
                        "  LEFT JOIN (SELECT " +
                        "               date_format(max(bd1.growth_date), '%Y-%m-%d') specialDate, " +
                        "               bd1.house_breed_id, " +
                        "               hb1.house_id " +
                        "             FROM s_b_breed_detail bd1 LEFT JOIN s_b_house_breed hb1 ON hb1.id = bd1.house_breed_id " +
                        "             WHERE hb1.farm_breed_id = " + FarmBreedId + " AND hb1.house_id = " + HouseId + ") ae ON 1 = 1 " +
                        "WHERE hb.farm_breed_id = " + FarmBreedId + " AND hb.house_id = " + HouseId + " AND " +
                        "      bd.growth_date BETWEEN date_sub(date_format(CASE WHEN hb.batch_status = '01' " +
                        "        THEN if((ae.specialDate) > curdate(), curdate(), ae.specialDate) " +
                        "                                                                           ELSE hb.market_date END, '%Y-%m-%d'), " +
                        "                                                               INTERVAL 31 DAY) AND date_format( " +
                        "          CASE WHEN hb.batch_status = '01' " +
                        "            THEN if(ae.specialDate > curdate(), curdate(), ae.specialDate) " +
                        "          ELSE hb.market_date END, '%Y-%m-%d') " +
