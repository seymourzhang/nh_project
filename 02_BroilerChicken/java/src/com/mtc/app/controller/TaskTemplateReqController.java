...
public class TaskTemplateReqController {
	...
	public void addTsk(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			if(TaskType.equals("01")) {
				sSDUserOperationService.insert(SDUserOperationService.MENU_TASK_SETTING_YOUNG_PREPARE, SDUserOperationService.OPERATION_ADD, id_spa);
				mLogger.info("==========入雏前准备操作信息：新增，导入完毕");
			}else if(TaskType.equals("02")) {
				sSDUserOperationService.insert(SDUserOperationService.MENU_TASK_SETTING_YOUNG_DAILY, SDUserOperationService.OPERATION_ADD, id_spa);
				mLogger.info("==========入雏后日常工作操作信息：新增，导入完毕");
			}else if(TaskType.equals("03")) {
				sSDUserOperationService.insert(SDUserOperationService.MENU_TASK_SETTING_LIGHT, SDUserOperationService.OPERATION_ADD, id_spa);
				mLogger.info("==========光照程序操作信息：新增，导入完毕");
			}else if(TaskType.equals("04")) {
				sSDUserOperationService.insert(SDUserOperationService.MENU_TASK_SETTING_IMMUNE_DRUG, SDUserOperationService.OPERATION_ADD, id_spa);
				mLogger.info("==========免疫用药操作信息：新增，导入完毕");
			}else if(TaskType.equals("05")) {
				sSDUserOperationService.insert(SDUserOperationService.MENU_TASK_SETTING_TEMPORARY_TASK, SDUserOperationService.OPERATION_ADD, id_spa);
				mLogger.info("==========其他临时任务操作信息：新增，导入完毕");
			}
	...
	public void updateTsk(HttpServletRequest request,
		...
		try {
			...
			String TaskStatus = prarObj.getString("TaskStatus");
			if(TaskType.equals("01")) {
				sSDUserOperationService.insert(SDUserOperationService.MENU_TASK_SETTING_YOUNG_PREPARE, SDUserOperationService.OPERATION_UPDATE, id_spa);
				mLogger.info("==========入雏前准备操作信息：修改，导入完毕");
			}else if(TaskType.equals("02")) {
				sSDUserOperationService.insert(SDUserOperationService.MENU_TASK_SETTING_YOUNG_DAILY, SDUserOperationService.OPERATION_UPDATE, id_spa);
				mLogger.info("==========入雏后日常工作操作信息：修改，导入完毕");
			}else if(TaskType.equals("03")) {
				sSDUserOperationService.insert(SDUserOperationService.MENU_TASK_SETTING_LIGHT, SDUserOperationService.OPERATION_UPDATE, id_spa);
				mLogger.info("==========光照程序操作信息：修改，导入完毕");
			}else if(TaskType.equals("04")) {
				sSDUserOperationService.insert(SDUserOperationService.MENU_TASK_SETTING_IMMUNE_DRUG, SDUserOperationService.OPERATION_UPDATE, id_spa);
				mLogger.info("==========免疫用药操作信息：修改，导入完毕");
			}else if(TaskType.equals("05")) {
				sSDUserOperationService.insert(SDUserOperationService.MENU_TASK_SETTING_TEMPORARY_TASK, SDUserOperationService.OPERATION_UPDATE, id_spa);
				mLogger.info("==========其他临时任务操作信息：修改，导入完毕");
			}
			if(TaskTemplateReqController.checkAgeInfos(AgeInfos)){
	...
	public void queryTskDetail(HttpServletRequest request,
		...
		try {
			...
			if (tSBFarmTask != null) {
				...
				if(TaskType.equals("01")) {
					sSDUserOperationService.insert(SDUserOperationService.MENU_TASK_SETTING_YOUNG_PREPARE, SDUserOperationService.OPERATION_SELECT, userId);
					mLogger.info("==========入雏前准备详细操作信息：查询，导入完毕");
				}else if(TaskType.equals("02")) {
					sSDUserOperationService.insert(SDUserOperationService.MENU_TASK_SETTING_YOUNG_DAILY, SDUserOperationService.OPERATION_SELECT, userId);
					mLogger.info("==========入雏后日常工作详细操作信息：查询，导入完毕");
				}else if(TaskType.equals("03")) {
					sSDUserOperationService.insert(SDUserOperationService.MENU_TASK_SETTING_LIGHT, SDUserOperationService.OPERATION_SELECT, userId);
					mLogger.info("==========光照程序详细操作信息：查询，导入完毕");
				}else if(TaskType.equals("04")) {
					sSDUserOperationService.insert(SDUserOperationService.MENU_TASK_SETTING_IMMUNE_DRUG, SDUserOperationService.OPERATION_SELECT, userId);
					mLogger.info("==========免疫用药详细操作信息：查询，导入完毕");
				}else if(TaskType.equals("05")) {
					sSDUserOperationService.insert(SDUserOperationService.MENU_TASK_SETTING_TEMPORARY_TASK, SDUserOperationService.OPERATION_SELECT, userId);
					mLogger.info("==========其他临时任务详细操作信息：查询，导入完毕");
				}
	...
	public void queryTskList(HttpServletRequest request,
		...
		try {
			...
			if(TaskType.equals("01")) {
				sSDUserOperationService.insert(SDUserOperationService.MENU_TASK_SETTING_YOUNG_PREPARE, SDUserOperationService.OPERATION_SELECT, userId);
				mLogger.info("==========入雏前准备操作信息：查询，导入完毕");
			}else if(TaskType.equals("02")){
				sSDUserOperationService.insert(SDUserOperationService.MENU_TASK_SETTING_YOUNG_DAILY, SDUserOperationService.OPERATION_SELECT, userId);
				mLogger.info("==========入雏后日常工作操作信息：查询，导入完毕");
			}else if(TaskType.equals("03")){
				sSDUserOperationService.insert(SDUserOperationService.MENU_TASK_SETTING_LIGHT, SDUserOperationService.OPERATION_SELECT, userId);
				mLogger.info("==========光照程序操作信息：查询，导入完毕");
			}else if(TaskType.equals("04")){
				sSDUserOperationService.insert(SDUserOperationService.MENU_TASK_SETTING_IMMUNE_DRUG, SDUserOperationService.OPERATION_SELECT, userId);
				mLogger.info("==========免疫用药操作信息：查询，导入完毕");
			}else if(TaskType.equals("05")){
				sSDUserOperationService.insert(SDUserOperationService.MENU_TASK_SETTING_TEMPORARY_TASK, SDUserOperationService.OPERATION_SELECT, userId);
				mLogger.info("==========其他临时任务操作信息：查询，导入完毕");
			}
