...
//						weatherResult = RequestWeather.requestWeather(tTempInfo.get(0).get("bak2").toString(),null, null);
			...
			try {
				...
				resJson.put("Exception", "天气预报更新出错，请稍后再试。");
	...
	public void reqOverview(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
            ...
            String sql = "SELECT" +
					"  if(max(datediff(case when hb.batch_status = '01' then curdate() else hb.market_date end, hb.place_date)) < 0, 0, max(datediff(case when hb.batch_status = '01' then curdate() else hb.market_date end, hb.place_date)))         feed_days, " +
					"  (SELECT count(*) " +
					"   FROM s_d_house h " +
					"   WHERE h.farm_id = farm.id)                     house_num," +
					"  (SELECT count(*) " +
					"   FROM s_b_devi_house dh " +
					"   WHERE dh.farm_id = farm.id)                    device_num," +
					"  (SELECT count(*) " +
					"   FROM s_b_biz_message bm " +
					"   WHERE bm.user_id = " + UserId + " AND status = '01') msg " +
					"FROM s_d_farm farm " +
					"  LEFT JOIN s_b_farm_breed fb ON fb.farm_id = farm.id AND fb.batch_status = '01' " +
					"  LEFT JOIN s_b_house_breed hb ON hb.farm_breed_id = fb.id " +
            ...
            for (HashMap<String, Object> stringObjectHashMap : message) {
               ...
               /* if (stringObjectHashMap.get("device_num") == null){
                    OverView.put("deviceNum", "-");
                    resJson.put("OverView", OverView);
                    resJson.put("Error", "暂无绑定设备信息！");
                    resJson.put("Result", "Fail");
                } else if (stringObjectHashMap.get("feed_days") == null){
                    resJson.put("OverView", OverView);
                    resJson.put("Error", "暂无批次信息！");
                    resJson.put("Result", "Fail");
                } else  if (stringObjectHashMap.get("house_num") == null){
                    resJson.put("OverView", OverView);
                    resJson.put("Error", "暂无农场信息！");
                    resJson.put("Result", "Fail");
                } else {*/
                    resJson.put("OverView", OverView);
