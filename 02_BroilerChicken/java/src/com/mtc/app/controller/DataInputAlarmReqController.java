    ...
    public void DataInputAlarm(HttpServletRequest request, HttpServletResponse response){
        mLogger.info("=====Now start executing DataInputAlarmReqController.DataInputAlarm");
			...
			try {
				...
				//** 业务处理开始，查询、增加、修改、或删除 **//*
				String SQL = "SELECT a.house_id,s_f_getHouseName(a.house_id) as house_name from s_b_house_breed a where a.farm_breed_id = " + FarmBreedId +
						" and exists(SELECT 1 from s_b_breed_detail b where a.id = b.house_breed_id and" +
						" b.growth_date < curdate()" +
						" and ifnull(b.cur_cd,0) = 0" +
						" and ifnull(b.cur_feed,0) = 0" +
						" and ifnull(b.cur_weight,0) = 0" +
						" and ifnull(b.num_bak1,0) = 0 )" +
