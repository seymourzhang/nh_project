			...
			Sparraw.ajaxPost('/rep/weekWeight/DFRRep_v2.action', params, function(data){
				console.log("批次号对比");
						if (data.ResponseDetail.Result == "Success" && data.ResponseDetail.DCDatas.length != 0){
							...
							if (Maxage <= 6) {
                               $scope.cullDeathRateData.xData  = [1,2,3,4,5,6];
							}else{
                                for (var i = 0; i < Maxage; i++) {
                                	$scope.cullDeathRateData.xData[i] = i+1;
                                }
							}
							//$scope.cullDeathRateData.xData  = ["第1周","第2周","第3周","第4周","第5周","第6周","第7周"];
