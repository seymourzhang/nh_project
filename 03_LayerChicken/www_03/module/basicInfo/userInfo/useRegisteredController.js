...
.controller("UseRegisteredCtrl",function($scope, $state, $http, $ionicPopup, AppData) {
	...
	$scope.save = function(){
	    ...
	    var required = ['name','phone','password','confirmPassword','role'];
	   	...
	   	//判断手机号码格式
	   	if($scope.sparraw_user_temp.userinfo.phone && /^1[3|4|7|5|8]\d{9}$/.test($scope.sparraw_user_temp.userinfo.phone)){
		//对的
		console.log("输入正确");
		} else{
		//不对
		return Sparraw.myNotice("请输入正确手机号码");
	   	...
	   	if (!checkInput($scope.sparraw_user_temp.userinfo.role,'选择','角色')) {return;}
