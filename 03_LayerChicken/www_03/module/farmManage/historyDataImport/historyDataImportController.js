...
.controller("historyDataImportCtrl",function($scope, $state, $http, AppData,uiGridConstants){
	...
	$scope.drowTable = function(){
		...
		var headerCellTem4 = "<div style='color:black;text-align:center;position:relative; left:0px; top:5px;'><p>产蛋重量<br>公斤</p></div>";
		...
		$scope.gridOptions.columnDefs = [		
			...
			{name:'curLayWeight',displayName:'产蛋重量<br>公斤',width:'20%',enableColumnMenu:false,headerCellTemplate:headerCellTem4},
	...
	$scope.save = function(){
		...
		for (var i = 0; i < $scope.uploadDataInput.length; i++) {
			...
			$scope.uploadDataInput[i].curLayWeight = Common_NulltoZero($scope.uploadDataInput[i].curLayWeight);
