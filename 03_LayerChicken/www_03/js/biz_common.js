...
var biz_common_judgeRegistInfo = function(ionicPopup, state, page) {
    if (sparraw_user.userinfo.role == 1 || sparraw_user.userinfo.role == 2) {
...
// 重新获取该用户最新数据
var biz_common_getLatestData = function(state, nextPage,nextFunction) {
...
//格式化图表数据
function setChartParams(compareType,data,lineName) {
    var returnTempLineConfig = [] ;
    console.log(data);
    ...
    return returnTempLineConfig;
};


//判断输入内容
function checkInput(inputValue,inputMode,inputName){
    if (inputValue == "" || inputValue == null) {
        Sparraw.myNotice('请' + inputMode + '正确的' + inputName + '!');
        return false;
    }else{
        return true;
    }
