    ...
    public void queryAlarmLog(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (!histHist.isEmpty()) {
            for (PageData pageData : histHist) {
                ...
                temp.put("realValue", pageData.get("actual_value").toString().replace(" 度","").replace(" ml/m3", ""));
                temp.put("targetValue", pageData.get("set_value").toString().replace(" 度","").replace(" ml/m3", ""));
                ...
                temp.put("deal_time", sdf1.format(sdf.parse(pageData.get("remove_time").toString())));
