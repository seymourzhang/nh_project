...
public class FarmMobileAction extends BaseAction {
    ...
    public void deviceQuery(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            String DeviceCode = "";
            String mainId = "";
            PageData deviceList = farmService.findDeviceForOut(pd);
            if (!PubFun.isNull(deviceList)){
                DeviceCode = deviceList.get("device_code").toString();
                mainId = deviceList.get("main_id").toString();
    ...
    public void deviceSave(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            sensor.put("sensor_no", 1);
            sensor.put("sensor_code", "1000");
            sensor.put("show_column", "inside_temp1");
            SensorInfo.put(sensor);
            sensor = new JSONObject();
            sensor.put("sensor_no", 2);
            sensor.put("sensor_code", "1000");
            sensor.put("show_column", "inside_temp2");
            SensorInfo.put(sensor);
            sensor = new JSONObject();
            sensor.put("sensor_no", 3);
            sensor.put("sensor_code", "1000");
            sensor.put("show_column", "inside_temp10");
            SensorInfo.put(sensor);
            sensor = new JSONObject();
            sensor.put("sensor_no", 4);
            sensor.put("sensor_code", "1000");
            sensor.put("show_column", "inside_temp19");
            SensorInfo.put(sensor);
            sensor = new JSONObject();
            sensor.put("sensor_no", 5);
            sensor.put("sensor_code", "1000");
            sensor.put("show_column", "inside_temp20");
            SensorInfo.put(sensor);
