...
function settleColumns(){
    ...
    dataColumns = [{field: "farmName",
        ...
        formatter: function(value,row,index){
        	...
        	return "&nbsp;<button type='button' class='btn blue' style='display: inline;' onclick='settleSub(\""+row.batch_no+"\","+row.settle_status+","+row.id+","+row.gross_chicken_number+","+row.gross_chicken_weight+","+row.gross_male_chicken_number+","+row.gross_male_chicken_weight+",\""+row.farmName+"\")' >"+
        	status+"</button>"+
        	"&nbsp;<button type='button' class='btn blue' style='display: inline;' onclick='settleSub2(\""+row.batch_no+"\","+row.settle_status+","+row.id+","+row.gross_chicken_number+","+row.gross_chicken_weight+","+row.gross_male_chicken_number+","+row.gross_male_chicken_weight+",\""+row.farmName+"\")' >"+
        	"确认</button>";
        }
    }];
...
function settleSub(child_batch_no,value,id,gross_chicken_number,gross_chicken_weight,gross_male_chicken_number,gross_male_chicken_weight,farmName){
...
//	}
	$.ajax({
        ...
        success: function (result) {
            ...
            document.getElementById("js4").innerHTML =pd.munberCount +" 元/只";
            document.getElementById("js5").innerHTML =pd.weightCount +" 元/公斤";
            ...
            if(gross_male_chicken_number!=undefined){
            	objBatch.gross_male_chicken_number=gross_male_chicken_number;
                objBatch.gross_male_chicken_weight=gross_male_chicken_weight;
            }
        }
    });
}

function settleSub2(child_batch_no,value,id,gross_chicken_number,gross_chicken_weight,gross_male_chicken_number,gross_male_chicken_weight,farmName){
	objBatch.settle_id = id;
	objBatch.farmName5=farmName;
	objBatch.settle_status5=value;
	objBatch.child_batch_no5=child_batch_no;
...
//		str += '<div class="span2" align="left">';
		str += '<select id="settle_type" name="settle_type" style="width: 100px;" onchange="setGoodName2('+1+','+1+','+null+','+
		null+','+null+');">';
		...
		str += '<select id="good_id" name="good_id" style="width: 100px;" onchange="setSpec2('+$("#settle_type").val()+',0,0,0);">';
		str += '</select>';
		str += '</div>';
		str += '<div class="span3" align="right">';
...
//				editSettleData();
	            ...
	            document.getElementById("maoji3").style.display = "none";
	            document.getElementById("maoji4").style.display = "none";
	            settleSub(objBatch.child_batch_no5,objBatch.settle_status5,objBatch.settle_id,objBatch.gross_chicken_number,objBatch.gross_chicken_weight,objBatch.gross_male_chicken_number,objBatch.gross_male_chicken_weight,objBatch.farmName5);
...
//		str += '</div>';
//			str += '<div class="span2" align="left">';
			str += '<select id="settle_type" name="settle_type" style="width: 100px;" onchange="setGoodName2('+1+','+settle_type+','+
			...
			str += '<div class="span3" align="right">';
			str += '<span_customer2 >名称</span_customer2 >&nbsp;&nbsp;&nbsp;';
			str += '<select id="good_id" name="good_id" style="width: 100px;" onchange="setSpec2('+settle_type+','+good_id+','+
			good_unit+','+spec+')">';
			str += '</select>';
			str += '</div>';
			str += '<div class="span3" align="right">';
...
//		str += '<div class="span2" align="left">';
		...
		str += '</div>';
		str += '<div class="row-fluid">';
		str += '<div class="span5" align="left">';
		str += '<span_customer2 >结算包含(</span_customer2 >';
		str += '<input type="checkbox" name="f_m_flag" id="f_m_flag" value="0" checked="checked" style="margin-top: -2px;" onclick="gongmuSum();" />&nbsp;&nbsp;母鸡&nbsp;&nbsp;&nbsp;';
		str += '<input type="checkbox" name="f_m_flag" id="f_m_flag" value="1" style="margin-top: -2px;" onclick="gongmuSum();" />&nbsp;&nbsp;公鸡';
...
layer.open({
	  ...
	  yes: function(index){
		...
		var arr = new Array(); 
		var items = document.getElementsByName("f_m_flag");  
		var f_m_flag=0;
...
setGoodName(2,settle_type,good_id,good_unit,spec);
}else{
	setCorporationGood2(good_total_price);
}
//setTimeout("shezhi("+settle_type+","+good_id+","+good_unit+","+spec+");",1000);

}

//function shezhi(settle_type,good_id,good_unit,spec){
//	document.getElementById("settle_type").value =settle_type;
...
function gongmuSum(){
	...
	if(arr.length==2){
		document.getElementById("good_munber").value = objBatch.gross_chicken_number+objBatch.gross_male_chicken_number;
		document.getElementById("good_weight").value = objBatch.gross_chicken_weight+objBatch.gross_male_chicken_weight;
	}else if(arr.length==1){
		if(arr[0] == 0){
			document.getElementById("good_munber").value = objBatch.gross_chicken_number;
			document.getElementById("good_weight").value = objBatch.gross_chicken_weight;
		}else{
			document.getElementById("good_munber").value = objBatch.gross_male_chicken_number;
			document.getElementById("good_weight").value = objBatch.gross_male_chicken_weight;
		}
	}else{
		document.getElementById("good_munber").value = objBatch.gross_chicken_number;
		document.getElementById("good_weight").value = objBatch.gross_chicken_weight;
...
//			var goodMunber = $("#good_munber").val();
			if(settletype==2){
	            ...
	            if(settleType5=="2"){//鸡苗
	            	...
	            	if(good_id != null && good_id !=0){
	            		getSumsiliao(good_id);
	            	}else{
	            		getSumsiliao($("#good_id").val());
	            	}
	            }else if(settleType5=="4"){//疫苗药品
	            	if(good_id != null && good_id !=0){
	            		getSummiaoyao(good_id);
	            	}else{
	            		getSummiaoyao($("#good_id").val());
	            	}
	            }
			}
...
//统计饲料数量
function getSumsiliao(good_id) {
	$.ajax({
		type : "post",
		url : path + "/batch/getSumsiliao",
		data : {farm_id:objBatch.farm_id,send_batch_no:objBatch.child_batch_no5,good_id:good_id
		},
		dataType : "json",
		success : function(result) {
			var list = result.obj;
...
//统计苗药数量
function getSummiaoyao(good_id) {
	$.ajax({
		type : "post",
...
//	$("#spec_select").attr("disabled", true);
	...
	var url1 = "";
	if(settle_type >2){
		url1 = path + "/googs/getCorporationGood";
	}else{
		url1 = path + "/batch/getCorporationGood";
	}
	$.ajax({
		type : "post",
		url : url1,
...
function setCorporationGood2(good_total_price) {
	$.ajax({
		...
		success : function(result) {
			var list = result.obj;
			document.getElementById("good_price").value = list[0].chicken_price;
...
function setSpec2(settle_type,good_id,good_unit,spec) {
//	$("#spec_select").attr("disabled", true);
	$.ajax({
		type : "post",
		url : path + "/googs/getSpec2",
...
//			$("#spec_select").attr("disabled", false);
			setUnit2(settle_type,good_id,good_unit,spec);
		}
	});
}
...
function setUnit2(settle_type,good_id,good_unit,spec) {
	$.ajax({
		type : "post",
		url : path + "/googs/getUnit2",
		data : {good_id:$("#good_id").val()},
