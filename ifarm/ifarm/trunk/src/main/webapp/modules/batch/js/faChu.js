...
$(function () {
    ...
    $('#fileupload1').fileupload({
        ...
        add: function (e, data){
            ...
            $('#uploadButtonFacade1').show();
            $('#uploadButtonFacade1').button('loading');
            $('#uploadButton1').hide();
            data.submit();
        },
        done: function (e, data) {
        	$('#uploadButtonFacade2').button('reset');
            $('#uploadButtonFacade2').hide();
            $('#uploadButton2').show();
            var rt = '(' + data.result + ')';
            var json = eval(rt);
            reFlushData(currTabName);
    ...
    $('#fileupload2').fileupload({
        ...
        add: function (e, data){
            ...
            $('#uploadButtonFacade2').show();
            $('#uploadButtonFacade2').button('loading');
            $('#uploadButton2').hide();
            data.submit();
        },
        done: function (e, data) {
...
layer.open({
	  ...
	  yes: function(index){
		...
		param ={operation_date:$("input[name='operation_date']").val(),
				...
				bak:$("#bak").val(),
				company_id:$("#faChuCompanySelect").val()};
		$.ajax({
    ...
    if (document.getElementById('variety_id').options.length > 0) {
        $.ajax({
            ...
            success: function (result) {
                ...
                var rt = new Array();
                for (var key in dataList) {
                    var tmp = {factory_id: dataList[key].factory_id, factory: dataList[key].factory_name};
                    rt.push(tmp);
                }
