	...
	<script type="text/javascript">
	 $(document).ready(function(){
		 ...
		 $("input[name='user_password']").blur(function(){
				var pwd=new RegExp("(?![0-9A-Z]+$)(?![0-9a-z]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{8,}$");
				var pd=$("input[name='user_password']").val();
				if(pd==""){
					$('#addUser_msg').html("密码不能为空！");
				}
				else if(!pwd.test(pd)){
					$('#addUser_msg').html("密码至少由8个字符组成，并且必须包含至少1个大写字母、1个小写字母以及1个数字，请重新输入！");
				}else{
					$('#addUser_msg').html("");
				}
			  });
		  ...
		  <div class="row-fluid">
    		...
    		<div class="row-fluid">
										...
										<select id="user_type"  name="user_type" >
											<c:if test="${!empty userTypeList}">
												<c:forEach var="userType" items="${userTypeList}">
													<option value="${userType.biz_code }" <c:if test="${userList.user_type==userType.biz_code}">selected</c:if>>${userType.code_name }</option>
												</c:forEach>
											</c:if>
										</select>
								</div>
    		  ...
    		  <div class="span3" align="right">
				...
				<div class="span9" align="left">
					<div class="zTreeDemoBackground left" style="border:1px solid #e5e5e5;height: 110px;overflow:auto;">
