...
.controller("weeklyCtrl",function($scope, $state, $http, $ionicLoading,$ionicLoading, $stateParams, $ionicPopup, AppData) {
    ...
    $scope.GetTable = function(){
        ...
        /*for (var i = 0; i < headName.length; i++) {
       		if (i == 0) {
       			headerDiv.push("<div style='color:black;text-align:center;position:relative; left:0px; width:90px; top:0px;height:20px;'><p style='position:relative;top:10px;'>" + Level2header[i] + "</p></div>");
       		}else if (i % 2 != 0) {
       			headerDiv.push("<div style='position:relative;left:0px; width:180px;height:40px;'><div style='position:relative;left:0px; width:180px;height:20px;background:rgba(255,255,255, 1);'><p style='text-align:center;'>" + Level1header[i] + "</p></div><div style='position:absolute;left:0px; top:20px; width:90px;height:20px;'><p style='text-align:center;'>" + Level2header[i] + "</p></div><div style='position:absolute;left:90px; top:20px; width:90px;height:20px;'><p style='text-align:center;'>" + Level2header[i+1] + "</p></div></div>");
       		}else{
       			headerDiv.push("");
       		}
       	}
          ...
          if (data.ResponseDetail.Result == "Success") {
            ...
            for (var i = 0; i < data.ResponseDetail.dataInfo.length; i++) {
              ...
              $scope.weeklyData.weekData.push({
                  ...
                  "index4":data.ResponseDetail.dataInfo[i].intake_acc          ,
                  "index5":data.ResponseDetail.dataInfo[i].intake_sig          ,
                  "index6":data.ResponseDetail.dataInfo[i].water_week           ,
                  "index7":data.ResponseDetail.dataInfo[i].water_sig      ,
                  "index8":data.ResponseDetail.dataInfo[i].week_layer_num     ,
                  "index9":data.ResponseDetail.dataInfo[i].week_layer_rate           ,
                  "index10":data.ResponseDetail.dataInfo[i].male_body_weight   ,
                  "index11":data.ResponseDetail.dataInfo[i].female_body_weight
              })
            }
            $scope.GetTable();
