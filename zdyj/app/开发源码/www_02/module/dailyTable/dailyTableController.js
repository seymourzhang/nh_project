...
.controller("dailyTableCtrl",function($scope, $state, $ionicLoading, $http, AppData) {
	...
	$scope.goCullDeathRate = function(){
		$state.go("cullDeathRate");
	};
	$scope.goChickenWeightAnalyze = function(){
		$state.go("chickenWeightAnalyze");
	};
	$scope.goEggWeigLayRate = function(){
		$state.go("eggWeigLayRate");
	};
